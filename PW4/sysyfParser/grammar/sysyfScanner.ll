...
/* Regex abbreviations: */
/********TODO: add regular expression for MultilineComment, HexConst, FloatConst, SingleLineComment********/
MultilineComment	    "/*"([^\*]|(\*)*[^\*/])*(\*)*"*/"
Identifier              [_a-zA-Z][a-zA-Z0-9_]*
OctConst                ("0"[0-7]*)
DecConst                ([1-9][0-9]*)
HexConst                ("0"[xX][0-9a-fA-F]*)
DecFloatConst           ((([0-9]*"."[0-9]+)|([0-9]+"."))([eE][-+]?[0-9]+)?|([0-9]+([eE][-+]?[0-9]+)))
...
%%
 /* keyword */
 /********TODO: add keyword definition and action here********/
...
return      {return yy::sysyfParser::make_RETURN(loc);}
 ...
 /* token and action */
 /********TODO: add other token definition and action here********/
...
"["         {return yy::sysyfParser::make_LSQUARE(loc);}
"]"         {return yy::sysyfParser::make_RSQUARE(loc);}
...
{DecFloatConst}             {
                                std::string flo = yytext;
                                float sum = std::stof(flo);
...
{MultilineComment}          {
                                ...
                                if(lastnewline >= 0)
                                {
                                    for(int i = lastnewline + 1; i < len; i++)
                                        loc.columns(1);
                                }
                                loc.step();
                            }
