...
/*
class TriangleInst : public Instruction
{
public:
    TriangleInst()
    {
...
public:
    ...
    virtual void print() override
    {
        ...
        for (auto v : values)
        {
            ...
            */
            std::vector<std::string>::iterator iter = find(InstType.begin(), InstType.end(), v->getName());
            if (iter == InstType.end())
            {
                InstType.push_back(v->getName());
                cnt.push_back(1);
            }
            else
            {
                int i = iter - InstType.begin();
                cnt[i]++;
            }
        }
         ...
         */
        std::vector<std::string>::iterator iter = InstType.begin();
        std::cout << name << ": " << std::endl;
        for (; iter != InstType.end(); ++iter)
        {
            int i = iter - InstType.begin();
            std::cout << cnt[i] << " " << InstType[i] << std::endl;
        }
        for (auto v : values)
        {
